...
3. Создать двумерный массив `d` размером `13x18`. Вычислить его элементы по следующей формуле (где `x = x[j]`):
   - если `f[i] = 12`, то $d[i][j] = \arcsin\left(\frac{x - 4.5}{21}\right)$;
   - если `f[i] \in \{13, 14, 15, 16, 17, 18\}`, то $d[i][j] = \sqrt[3]{(\pi \cdot \tan(x))^2}$;
   - для остальных значений `f[i]`: $d[i][j] = \tan\left(e^{e^{\left(\frac{2}{3 \cdot x}\right)^2}}\right)$.
