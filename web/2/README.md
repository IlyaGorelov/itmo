...
[Link to frontend-only](https://se.ifmo.ru/~s502362/2/index.html)

# Гайд на httpd

1. Загрузить фронт и бэк на гелиос.
1. Скачать шамблон httpd.conf и закинуть, куда надо
1. Там ввести порт, путь к index.html (фронтэнд) и путь к бэкэнду (как надо ищите в презентации или спросите чат жбт)
1. Чтобы открыть сайт по адресу http://helios.cs.ifmo.ru:[PORT], вам нужно либо быть подключенным к WIFI от ПИИКТА, находясь в ИТМО. Либо настроить ВПН от ПИИКТА (**не от ИТМО**). Если не получается загрузить сайт, попробуйте входить по IP (http://192.168.10.80:[PORT])
   > [Ссылка на ВПН](https://se.ifmo.ru/ca/students.ovpn). Для авторизации используется имя пользователя и пароль. Для студентов доступ ограничен только сервером helios и другими серверами с адресами в подсети 192.168.10.0/24.
